...
K2_50runs<-read.table("data/Q_bemigen_K2.indfile",header=FALSE,
...
K2_50runs<-split(K2_50runs[,-c(1:5)],rep(1:50,each=54))
...
K3_50runs<-read.table("data/Q_bemigen_K3.indfile",header=FALSE,
                       blank.lines.skip=TRUE,sep="\t")
#import the column order for the best CLUMPP "permutation"
...
#then we split the dataframe in as many repetition that has been made
#by the number of individuals (here 54)
K3_50runs<-split(K3_50runs[,-c(1:5)],rep(1:50,each=54))
#reordering the columns so that the different repetition colorization fit
for (i in 1:50){
...
K4_50runs<-read.table("data/Q_bemigen_K4.indfile",header=FALSE,
                       blank.lines.skip=TRUE,sep="\t")
#import the column order for the best CLUMPP "permutation"
...
#then we split the dataframe in as many repetition that has been made
#by the number of individuals (here 54)
K4_50runs<-split(K4_50runs[,-c(1:5)],rep(1:50,each=54))
#reordering the columns so that the different repetition colorization fit
for (i in 1:50){
  K4_50runs[[i]]<-K4_100runs[[i]][as.numeric(K4_colorder[i,])]
}
#importing the order of the run so that the different repetition 
#corresponding to the same clustering solution followed each other
...
#then we split the dataframe in as many repetition that has been made
#by the number of individuals (here 54)
K2_50runs<-split(K2_50runs[,-c(1:5)],rep(1:50,each=1509))
#reordering the columns so that the different repetition colorization fit
for (i in 1:50){
  K2_50runs[[i]]<-K2_50runs[[i]][as.numeric(K2_colorder[i,])]
}
...
#then we split the dataframe in as many repetition that has been made
#by the number of individuals (here 54)
K3_50runs<-split(K3_50runs[,-c(1:5)],rep(1:50,each=1509))
#reordering the columns so that the different repetition colorization fit
for (i in 1:50){
  K3_50runs[[i]]<-K3_50runs[[i]][as.numeric(K3_colorder[i,])]
}
